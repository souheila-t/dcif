...
public class GreaterThan extends Constraint {
  ...
  /**
   * Construct a greater-than constraint for equality handling.
   ...
   */
  public GreaterThan(Env env, Node node, int type, Term x, Term y) {
    super(env, node, type);
    this.x = x;
    this.y = y;
  }
  ...
  protected int isSatisfiable() {
    ...
    if (ret == TermWeight.TRUE || ret == TermWeight.SAME) 
      return UNSAT;
    if (weight1.isGreaterThan(weight2) == TermWeight.TRUE)
      return SAT;
    return UNKNOWN;
  }
