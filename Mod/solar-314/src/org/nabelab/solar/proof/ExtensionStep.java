...
import org.nabelab.solar.operator.Extension;
 ...
 *
 */
public class ExtensionStep extends ProofStep {

  /**
   * Constructs a extension step.
   * @param env    the environment.
   * @param clause the tableau clause.
   */
  public ExtensionStep(Env env, Clause clause) {
    super(env);
    this.clause = clause;
  }
  ...
  public Operator convert(Tableau tableau, Node node) {
    ...
    Literal target = Literal.newOffset(clause.get(0), offset); 
    Subst   subst  = node.getLiteral().isCompUnifiable(target);
    if (subst == null)
      return null;
    PClause pclause = new PClause(clause, 0);
    return new Extension(env, node, new Unifiable<PClause>(subst, pclause, offset));    
  }
  ...
  public String toString() {
    return "extension with " + clause.toSimpString();
  }
